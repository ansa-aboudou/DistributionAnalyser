...
def page_explore():
    ...
    with make_expanders("Select distribution"):
        ...
        slider_size = st.slider('Population size',
                                           min_value = 1,
                                           value = 400,
                                           max_value = 800,
                                           step = 1)
    ...
    def get_multi_parameters(*c_params):
        ...
        *c_params : a list of parameters of the distribution function.
        ...
        for j, param in enumerate(c_params):
            ...
            dict_distibution = {"mean": mean, "std":std, "select_distribution":select_distribution, "param_distribution":c_params[0], "p_0":p_0, "p_over":p_over, "overachiev_distribution":"uniform", "overachiev_param":{"loc": p_over_loc, "scale": p_over_scale}}
            ...
            if select_boxplot:
                ...
                if select_hist:
                    if select_hist_frequency:
                        Figure.histogram(self, ax=ax[0], density=False)
                    else:
                        Figure.histogram(self, ax=ax[0], density=True)
